...
function App(props) {
  ...
  const onLog = ({ passwordInput, emailInput }) => {
    return Auth.authorize(passwordInput, emailInput)
      .then((res) => {
        if (!res || !res.token)
      ...
      case "/main":
        return (
          <div className="header__auth">
            <p className="header__email" style={{ paddingRight: "24px" }}>
              {email}
            </p>
            <Link className=" auth__link" to="/sign-up" onClick={onSignOut}>
              Выйти
            </Link>
          </div>
        );
      ...
      <CurrentUserContext.Provider value={userInfo}>
        ...
        <Switch>
          ...
          <Route path="/sign-up">
            <Register
              onReg={onReg}
              onInfoTool={setIsInfoToolTipOpen}
              setResStatus={setResStatus}
            >
              <p className="auth__span">
                Уже зарегистрированы?
                <Link className="auth__span auth__link" to="/sign-in">
                  {" "}
                  Войти
                </Link>
              </p>
